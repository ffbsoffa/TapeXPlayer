...
//
//   macOS          — subprocess FullVT (scale_vt + h264_videotoolbox) → AME → libx264
...
public:
    ...
    // Verify a finished proxy is frame-locked to the source: the proxy's frame count must
    // match the SOURCE's real frame count (falling back to duration×fps only when the source
    // frame count is unavailable). Comparing against the real source count — not a duration×fps
    // estimate — avoids falsely rejecting a good proxy when the estimate is a few frames off the
    // truth. Cheap (container metadata only). Returns true if trustworthy (or source unknown).
