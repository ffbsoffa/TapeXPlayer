...
#ifdef __APPLE__
#include "../../objc_proxy_encoder.hpp"
#endif
                                 ...
                                 double sourceDuration, double sourceFps)
{
    ...
    if (sourceFps > 1.0) {
        int64_t expected = std::llround(sourceDuration * sourceFps);
        int64_t actual   = CountFrames(proxyPath);
        if (actual > 0) {
            int64_t diff = std::llabs(actual - expected);
            if (diff > 3) {   // ±3 slack for duration rounding / edit-list edge
                std::cerr << "[ProxyConverter] ❌ VERIFY FAILED: proxy " << actual
                          << " frames vs source ~" << expected << " (diff " << diff
                          << " > 3) — frames were dropped" << std::endl;
                return false;
            }
            std::cout << "✅ [Proxy] Verified frame-exact (" << actual << " vs ~"
                      << expected << " frames)" << std::endl;
            return true;
        }
...
int ProxyConverter::OptimalSegments(double durationSeconds)
{
    ...
    // Under 3 minutes: 2 segments (concat overhead matters more)
    if (durationSeconds < 180.0) return std::min(byCore, 2);
...
bool ProxyConverter::ConvertSingle(const Params& params)
{
    ...
    auto tryMode = [&](EncodeMode mode, const char* name) -> bool {
        ...
        bool ok = ExecuteFFmpeg(BuildCommand(params, dest, -1.0, -1.0, mode),
                  ...
                  VerifyProxy(dest.string(), totalDuration, vfps);
        if (!ok) return false;
...
bool ProxyConverter::ConvertParallel(const Params& params, int numSegments)
{
    ...
    if (ok && !VerifyProxy(params.outputPath.string(), totalDuration, vfps)) {
