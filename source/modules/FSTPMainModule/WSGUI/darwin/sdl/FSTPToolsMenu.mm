...
- (void)mouseDown:(NSEvent *)event {
    ...
    if (row >= 0) {
        // Check Command+click - edit marker
        if (event.modifierFlags & NSEventModifierFlagCommand) {
            FSTP_MemoryLocationData data;
            if (FSTP_GetMemoryLocationData((int)row, &data)) {
                NSLog(@"✏️ Cmd+Click - Editing Memory Location #%d: %s", data.id, data.name);

                // Get current active player and time
                int active_player = GetActivePlayerID();
                if (active_player < 0) {
                    active_player = 0; // Fallback to player 0
                }
                double current_time = GetInstancePosition(active_player);

                // Show edit dialog with existing location data
                ShowSwiftUIMemoryLocationDialogEdit(active_player, current_time, data.id);
        ...
        // Check Alt/Option - delete marker
...
- (void)exportMemoryLocationsAction:(id)sender {
    NSLog(@"📊 Exporting Memory Locations to CSV...");

    NSSavePanel* panel = [NSSavePanel savePanel];
    [panel setNameFieldStringValue:@"memory_locations.csv"];
    [panel setMessage:@"Export Memory Locations to CSV"];
    ...
    if ([panel runModal] == NSModalResponseOK) {
        ...
        if (FSTP_ExportMemoryLocationsToCSV([filepath UTF8String])) {
            NSLog(@"✅ Memory Locations exported to CSV: %@", filepath);

            // Show success alert
            NSAlert* alert = [[NSAlert alloc] init];
            [alert setMessageText:@"Export Successful"];
            [alert setInformativeText:[NSString stringWithFormat:@"Memory Locations exported to:\n%@", filepath]];
            [alert addButtonWithTitle:@"OK"];
            ...
            NSAlert* alert = [[NSAlert alloc] init];
            [alert setMessageText:@"Export Failed"];
            [alert setInformativeText:@"Failed to export Memory Locations to CSV"];
            [alert addButtonWithTitle:@"OK"];
